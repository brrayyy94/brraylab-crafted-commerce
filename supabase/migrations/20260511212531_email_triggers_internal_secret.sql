...
BEGIN
  ...
  ELSE
    UPDATE vault.secrets
      SET secret = 'brraylab-email-secret-2026-xK9mP2qL'
      WHERE name = 'send_email_internal_secret';
  END IF;
END
$$;

-- Helper to fetch the decrypted internal secret from Vault.
...
-- Update welcome-email trigger to include the internal secret header.
...
-- Trigger function: order created -> notify customer + admin
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'handle_order_created failed: %', SQLERRM;
  END;
  RETURN NEW;
END;
$fn$;
...
-- Trigger function: order status changed -> notify customer
...
BEGIN
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING 'handle_order_status_changed failed: %', SQLERRM;
    END;
  END IF;
  RETURN NEW;
END;
